...
class PlanningGraph():
        ...
        for action in self.all_actions:
            ...
            if proposed.prenodes.issubset(self.s_levels[level]):
                for literal in self.s_levels[level]:
                    if literal in proposed.prenodes:
                        proposed.parents.add(literal)
                        literal.children.add(proposed)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        if node_s1 == PgNode_s(node_s2.symbol, not node_s2.is_pos):
            return True
        if node_s2 == PgNode_s(node_s1.symbol, not node_s1.is_pos):
            return True
        return False
        ...
        if self.problem.goal:
            for g in self.problem.goal:
                ...
                for i in range(len(self.s_levels)):
                    for literal in self.s_levels[i]:
                        if (literal.symbol == g) and (literal.is_pos):
                            level_sum += i
                            found = True
                            break
                    if found:
