...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        for n in range(self.min_n_components, self.max_n_components+1):
            split_method = KFold(n_splits=folds, random_state=self.random_state)
            total_ll = 0.0
            successful_models = 0.0
            for train_idx, test_idx in split_method.split(self.sequences):
                ...
                try:
                    ...
                    if self.verbose:
                        print("model created for {} with {} states".format(self.this_word, n))
                    total_ll += hmm_model.score(test_X, test_lengths)
                    successful_models += 1
                except:
                    if self.verbose:
                        print("failure on {} with {} states".format(self.this_word, n))
            ...
            if successful_models == 0:
                ...
                average_ll[n] = total_ll/successful_models
        best = -math.inf
        best_n = 0
        for n in range(self.min_n_components, self.max_n_components+1):
            if average_ll[n] is not None:
                if average_ll[n] > best:
                    best = average_ll[n]
                    best_n = n
        if best == -math.inf:
            return None
